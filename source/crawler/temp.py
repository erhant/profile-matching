...
data = {}
users = []
with open('twitter_matchedusers.txt') as my_file:
    for line in my_file:
        users.append(line)
print(len(users))
...
# Twitter
# for i in range(141,172):
# 	data[users[i]] = twitter_extract(driver, users[i])
# 	save_as_json(users[i]+".txt", data[users[i]])
